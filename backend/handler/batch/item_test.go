...
func TestItemBatchHandler_UpdateAll(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	mockItemBatchService := mock_service.NewMockItemBatchService(ctrl)
	handler := NewItemBatchHandler(mockItemBatchService)
	...
	t.Run("[正常系] バッチ処理に成功したとき、成功したことをスラック通知すること", func(t *testing.T) {
		...
		mockItemBatchService.EXPECT().UpdateAll().Return(nil)
		handler.UpdateAll()
	})
	...
	t.Run("[異常系] バッチ処理に失敗したとき、失敗したことをスラック通知すること", func(t *testing.T) {
		...
		mockItemBatchService.EXPECT().UpdateAll().Return(errors.New("エラー内容"))
		handler.UpdateAll()
	})
}
